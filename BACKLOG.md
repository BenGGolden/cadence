...
## Linear MCP detection misses the claude.ai connector's display-name format

**Idea**: teach
[scripts/detect_linear_mcp_namespace.py](./scripts/detect_linear_mcp_namespace.py)
to recognise the claude.ai workspace connector when detecting from
`claude mcp list` output, not just locally-installed servers.

**Why**: the detector's `_NAME_RE` is anchored to line start
(`^([A-Za-z0-9_-]*[Ll]inear[A-Za-z0-9_-]*)`) and assumes the line
begins with a bare namespace token (`linear`, `linear-server`). But the
actual `claude mcp list` output for the claude.ai connector is a
human display name:
...
The regex matches `claude`, hits the `.` before reaching `linear`, and
fails — so detection falls through to `.mcp.json`, fails again, and the
operator lands on the "detection failed" placeholder path. Yet the real
namespace is plainly `claude_ai_Linear` (it shows up in this session's
tool names as `mcp__claude_ai_Linear__…`). The detector's own docstring
lists `claude_ai_Linear` as one of the three supported namespaces, so
this is a gap against its stated contract — the display-name → namespace
mapping (`claude.ai Linear` → `claude_ai_Linear`) is the missing piece.

**Shape sketch**:

- Either scan anywhere in the line for a `linear`-containing token (not
  just line-start), or special-case the `claude.ai <Name>` display
  format and map it to the underscore namespace.
- Confirm the exact `claude mcp list` line format for a locally-installed
  `linear` / `linear-server` server before changing the regex — the
  current anchored form was clearly written against that case and must
  keep matching it. (Only the claude.ai display-name line was confirmed
  during the spike; the bare-token form is assumed, not verified.)
- Extend `tests/` with a fixture covering the `claude.ai Linear:` line.

**Why not now**: surfaced incidentally during the init-scaffold spike
(confirming `claude mcp list` runs cleanly as a nested CLI child), not
during operator use. It's a pre-existing detection gap, orthogonal to
the prose-to-script determinism work — the operator can still hand-edit
the namespace via the placeholder path. Pick it up when an operator on
the claude.ai connector hits the silent fall-through, or alongside the
next change to `detect_linear_mcp_namespace.py`.

**Discussed in**: conversation on 2026-05-28 — the init-scaffold-driver
spike ran `claude mcp list` and the output didn't match the detector's
expected format.
