...
- `templates/hooks/` — ten Python files copied to the consumer's
  ...
  `audit_linear_writes.py`); seven are deterministic helpers the dispatch
  ...
  `render_status_report.py`). All ten are always overwritten on init —
  they are plugin-owned executables, not user config.
